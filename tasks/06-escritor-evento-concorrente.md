...
- [x] `write_event()` grava um evento válido no arquivo correto (`events/<writer_id>.jsonl`) e
      rejeita (levanta erro) um evento que falha a validação de schema — tested by: teste
      unitário com evento válido e um inválido.
- [x] `write_event()` nunca reescreve/trunca o arquivo, só append — tested by: teste unitário
      chamando `write_event()` duas vezes e validando que ambas as linhas estão presentes.
- [x] Múltiplos escritores (simulados via threads ou processos) gravando eventos
...
- [x] `rebuild_timeline()` mescla eventos de múltiplos arquivos de escritor numa ordem
...
- [x] `rebuild_timeline()` é idempotente (rodar duas vezes no mesmo run_dir produz o mesmo
