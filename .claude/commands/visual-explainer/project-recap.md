...
4. **Decision context.** Read recent commit messages for rationale. If running in the same session as recent work, mine the conversation history. Read any plan docs, RFCs, or ADRs in the project directory (`docs/adr/` in this harness).
...
4. **Decision log** — key design decisions from the time window. Extracted from commit messages, conversation history, plan docs, progress docs, and ADRs (`docs/adr/`). Each entry: what was decided, why, what was considered. This is the highest-value section for fighting cognitive debt — the reasoning that evaporates first.
