...
**`AGENTS.md` vs `CLAUDE.md`:** O `CLAUDE.md` é Claude Code-específico (carrega rules, skills, MCP). O
`AGENTS.md` é o equivalente portátil para outros editores — contém stack, comandos, invariantes e
"onde fica o quê", mas sem referências a `.claude/` (que não existe nesses editores). Gere ambos
sempre que o projeto tiver `CLAUDE.md`.

**`CONTEXT.md`:** Crie um esqueleto vazio com o template padrão e a instrução de preenchimento via
